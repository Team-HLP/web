...
export default function SessionDetailPage() {
  ...
  const [windowSize, setWindowSize] = useState(20);       // pupil 이동평균 구간

  /* 확대 모달 */
  const [zoom, setZoom] = useState({
    show: false, side: 'left', data: [], baseline: 0, range: [0, 0],
  });

  /* EEG 토글 – delta만 기본 ON */
  const [eegVisible, setEegVisible] = useState(
    EEG_BANDS.reduce((o, k, i) => ({ ...o, [k]: i === 0 }), {})
  );
  ...
  const eegLines = EEG_BANDS
    .map(b => ({
      id: b, color: EEG_COLORS[b],
      data: (gameData.eeg_data ?? []).map(d => ({ x: d.time_stamp, y: d[b] }))
    }))
  ...
  const common = {
    ...
    pointSize: 0, curve: 'monotoneX', lineWidth: 1.5, useMesh: true,
  };
  ...
  const openZoom = side => slice => {
    ...
    const raw = rec.filter(r => r.time_stamp >= minT && r.time_stamp <= maxT)
      .map(r => ({ x: r.time_stamp, y: r.pupil_size[side] }));
    setZoom({
      show: true, side, data: raw,
      baseline: baseline[side] ?? 0, range: [+minT.toFixed(1), +maxT.toFixed(1)]
    });
  };

  /* CSV */
  const downloadPupilCSV = () => makeCSVandDownload(
    `session_${gameId}_pupil.csv`,
    ['time_stamp', 'left_pupil', 'right_pupil'],
    rec.map(r => [r.time_stamp, r.pupil_size.left, r.pupil_size.right])
  );
  const downloadEegCSV = () => makeCSVandDownload(
    `session_${gameId}_eeg.csv`,
    ['time_stamp', ...EEG_BANDS],
    (gameData.eeg_data ?? []).map(d => [d.time_stamp, d.delta, d.theta, d.alpha, d.beta, d.gamma])
  );
    ...
    <Container className="mt-5 pb-4">
      ...
      <Row className="mb-4 align-items-center justify-content-between">
        ...
        <Col xs="auto">
          <ButtonGroup className="me-2">
            <Button size="sm" variant="outline-success" onClick={downloadPupilCSV}>Pupil CSV</Button>
            <Button size="sm" variant="outline-info" onClick={downloadEegCSV}>EEG CSV</Button>
          </ButtonGroup>
      ...
      <Card className="p-4 mb-4">
        <Row className="fw-semibold align-items-center">
          ...
          {/* ➊ 평활 구간 컨트롤 – 같은 Row 의 마지막 Col */}
          <Col xs="auto" className="d-flex align-items-center">
            <span className="me-2 fw-semibold">평활 구간:</span>
            <Form.Control
              type="number"
              value={windowSize}
      ...
      <Card className="p-4 mb-5">
        ...
        {eegLines.length
          ...
          : (<div className="text-muted py-4 text-center">표시할 EEG 밴드가 없습니다.</div>)
        }
      </Card>
      ...
      <Modal show={zoom.show} size="lg"
        ...
        <Modal.Header closeButton>
          <Modal.Title>
            {zoom.side === 'left' ? '왼쪽' : '오른쪽'} 동공 RAW 확대 · t ≈ {zoom.range[0]} – {zoom.range[1]}
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <div style={{ height: 300 }}>
